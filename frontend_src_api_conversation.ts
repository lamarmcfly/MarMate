...
const conversationService = {
  ...
  getConversationHistory: async (conversationId: string): Promise<ApiResponse<ConversationHistoryResponse>> => {
    const apiService = createApiService<ConversationHistoryResponse>('conversation');
    return apiService.getById(conversationId);
  },

  // Check conversation status
  checkConversationStatus: async (conversationId: string): Promise<ApiResponse<ConversationState>> => {
    const apiService = createApiService<ConversationState>('conversation');
    return apiService.custom<ConversationState>('get', `${conversationId}/status`);
  },

  // WebSocket connection helper
  createWebSocketConnection: (conversationId: string): WebSocket => {
    const wsUrl = import.meta.env.VITE_WS_URL || 'ws://localhost:5678';
    return new WebSocket(`${wsUrl}/conversation/${conversationId}`);
  },
};
